...
//{{AFX_MSG_MAP(CGridCtrl)
    ...
    ON_WM_SYSKEYDOWN()
	ON_CONTROL(CBN_SELCHANGE, IDC_INPLACE_CONTROL, OnComboChangeSelection)
//}}AFX_MSG_MAP
#ifndef _WIN32_WCE_NO_CURSOR
    ON_WM_SETCURSOR()
#endif
...
int CGridCtrl::InsertRow(LPCTSTR strHeading, int nRow /* = -1 */)
{
	...
	////change STARTS here
	// fix up column order
	m_arRowOrder.resize(m_nRows); // Reset Column Order
	for (int i = 0; i < m_nRows; i++)
	{
	m_arRowOrder[i] = i; 
	}
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    if (m_MouseMode == MOUSE_OVER_COL_DIVIDE) // sizing column
    {
        ...
        //rect.left  = start.x + 1;
        rect.left = start.x + (m_bAllowColHide ? 0 : 1);  //fixed the bug by Tobert: can't hide row or column
    ...
    else if (m_MouseMode == MOUSE_OVER_ROW_DIVIDE) // sizing row
    {
        ...
        //rect.top = start.y + 1;
        rect.top = start.y + (m_bAllowRowHide ? 0 : 1); //fixed the bug by Tobert: can't hide row or column
...
void CGridCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
    ...
    {
//      fixed by   benrose. bug: the check box on the fixed cell can't work well (wxl)
//		SendMessageToParent(m_idCurrentCell.row, m_idCurrentCell.col, GVN_SELCHANGED);
//		
//        CGridCellBase* pCell = GetCell(m_idCurrentCell.row, m_idCurrentCell.col);
//        if (pCell)
...
//        SendMessageToParent(m_LeftClickDownCell.row, m_LeftClickDownCell.col, NM_CLICK);
		...
		SendMessageToParent(m_LeftClickDownCell.row, m_LeftClickDownCell.col, GVN_SELCHANGED);
		
		CGridCellBase* pCell = GetCell(m_LeftClickDownCell.row, m_LeftClickDownCell.col);
		if (pCell)
			pCell->OnClick( GetPointClicked( m_LeftClickDownCell.row, m_LeftClickDownCell.col, point) );
		SendMessageToParent(m_LeftClickDownCell.row, m_LeftClickDownCell.col, NM_CLICK);
...
{
	// fixed the actual range, when some cells have already merged.
	int nMinRow= nStartRow,nMaxRow= nEndRow,nMinCol= nStartCol,nMaxCol= nEndCol;
	for (int row1=nStartRow;row1<=nEndRow;row1++) 
	{
		for (int col1=nStartCol;col1<=nEndCol;col1++) 
		{
			CGridCellBase *pCell= (CGridCellBase*)GetCell(row1,col1);
			ASSERT(pCell);
			if (pCell->IsMerged()) {
				CCellRange range=pCell->GetMergeRange();
				nMinRow = min(nMinRow,range.GetMinRow());
				nMinCol = min(nMinCol,range.GetMinCol());
				nMaxRow = max(nMaxRow,range.GetMaxRow());
				nMaxCol = max(nMaxCol,range.GetMaxCol());
			}
		}
	}
...
afx_msg void CGridCtrl::OnComboChangeSelection()
{
	CCellID next = m_idCurrentCell;
	SendMessageToParent(next.row, next.col, GVN_CB_SELCHANGE);
} 